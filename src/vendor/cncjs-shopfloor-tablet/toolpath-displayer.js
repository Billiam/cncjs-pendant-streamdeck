...
$(function () {
  ...
  var transformCanvas = function () {
    ...
    tp.textBaseline = 'top'
    tp.fillText(
      formatLimit(bbox.max.y),
      imageRight / 2,
      canvas.height - inset - imageTop
    )
    tp.textAlign = 'left'
    tp.textBaseline = 'center'
    tp.fillText(
      formatLimit(bbox.min.x),
      inset,
      canvas.height - inset - imageTop / 2
    )
    tp.textAlign = 'right'
    tp.textBaseline = 'center'
    tp.fillText(
      formatLimit(bbox.max.x),
      inset + imageRight,
      canvas.height - inset - imageTop / 2
    )
  ...
  ToolpathDisplayer.prototype.showToolpath = async function (
    gcode,
    wpos,
    mpos
  ) {
    ...
    await new Toolpath(bboxHandlers).loadFromLines(gcodeLines)
    transformCanvas()
    if (!bboxIsSet) {
      return
    }
    ...
    await new Toolpath(displayHandlers).loadFromLines(gcodeLines)
